...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            transformm = carla.Transform(carla.Location(x=-3.4, y=178.9, z=5), carla.Rotation(yaw=180))
...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        #self._control.throttle = 1.0 if keys[K_UP] or keys[K_w] else 0.5
        global flag_down
        ...
        #self._control.steer = round(self._steer_cache, 1)
        self._control.steer =steering_angle
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.recording:
            ...
            #cv2.resize(image, (227, 227), cv2.INTER_AREA)
            ...
            array2 = utils2016.preprocess(image)
            

            image = np.array([array2]) 
            ...
            #m7tageen n7wel l rgb aw nla2i tre2a 3'er imshow n3rd beha HSV image google it :")"
            steering_angle = float(model.predict(ih, batch_size=1)) 
            ...
            throttle = 0.5
            controller.apply_new(steering_angle,throttle,vkkv)
            print('{} {} {}'.format(steering_angle, throttle,vkkv))
            #array2=img_as_float(array2)\
            #array2=tf.cast(array2/255.0,tf.float32)
            #array2=skimage.img_as_float(array2)
            #img=Image.fromarray(ih,'RGB')
            #img.save('my.png')
            #img.show()
        ...
        while True:
            ...
            if controller.parse_events(client, world, clock):
               ...
               # BASE_DIR = "C:/Coursera/carla/PythonAPI/examples/"
                ...
                df.to_csv('files_path2.csv', header=None)

                
                a = pd.read_csv("files_path2.csv")
                b = pd.read_csv("files_path.csv")
                b = b.dropna(axis=1)
                merged = a.merge(b)
            ...
            global steering_angle
            global throttle
            #controller.apply_new(steering_angle,throttle)
            #print('i am here  ')
            world.tick(clock)
