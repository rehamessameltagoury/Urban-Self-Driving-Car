...
'''def batch_generator(data_dir, image_paths, steering_angles,speed_values, batch_size, is_training):
    global image
    """
    Generate training image give image paths and associated steering angles
    """
    ...
    while True:
        ...
        for index in np.random.permutation(image_paths.shape[0]):
            ...
            print(center)
            steering_angle = steering_angles[index]
            speed_value = speed_values[index]
            # argumentation
            if is_training and np.random.rand() < 0.6:
                #image,
                steering_angle = augument(data_dir, center,steering_angle)
                speed_value = augument(data_dir, center,speed_value)
                #******
            else:
                image = load_image(data_dir, center) 
            # add the image and steering angle to the batch
