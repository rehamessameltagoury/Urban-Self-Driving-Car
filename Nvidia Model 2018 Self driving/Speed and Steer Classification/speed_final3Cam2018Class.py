...
from utils3Cam2018Class import INPUT_SHAPE, batch_generator
...
def load_data(args):
    ...
    data_df = pd.read_csv(os.path.join(os.getcwd(), args.data_dir, 'comdataandpix.csv'), names=['index','center' ,'left','right', 'speed' ,'steering' ,'Throttle','brake' , 'Speed_Sequence','Speed_Classes'])
    #yay dataframes, we can select rows and columns by their names
    #we'll store the camera images as our input data
...
def train_model(concate_model, args,X_train_image, X_valid_image, Y_train_steer, Y_valid_steer , X_train_Sequence, X_valid_Sequence, Y_train_speed, Y_valid_speed ,Continue = 0):
    ...
    if Continue == False:
        ...
        #concate_model.compile(loss='mean_squared_error' ,metrics={'Steer': 'mse', 'Speed':'mse'}, optimizer=Adam(learning_rate=args.learning_rate))
        concate_model.compile(loss='mean_squared_error' ,metrics={'Steer': 'mse', 'Speed':'mse'}, 
...
def main():
    ...
    parser.add_argument('-d', help='data directory',        dest='data_dir',          type=str,   default='D:/Graduation project/Udacity simulator')
    parser.add_argument('-t', help='test size fraction',    dest='test_size',         type=float, default=0.2)
    parser.add_argument('-k', help='drop out probability',  dest='keep_prob',         type=float, default=0.5)
    parser.add_argument('-n', help='number of epochs',      dest='nb_epoch',          type=int,   default=30)
    parser.add_argument('-s', help='samples per epoch',     dest='samples_per_epoch', type=int,   default=200) #150
    ...
    if args.continue_learning:
        ...
        Total_Model = load_model('model.h5')
