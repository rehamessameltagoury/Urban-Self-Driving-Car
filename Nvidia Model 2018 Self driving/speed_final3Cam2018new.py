...
from utils3Cam2018new import INPUT_SHAPE, batch_generator
...
def load_data(args):
    ...
    data_df = pd.read_csv(os.path.join(os.getcwd(), args.data_dir, 'comdataandpix.csv'), names=['index','center' ,'left','right', 'speed' ,'steering' ,'Throttle','brake' , 'Speed_Sequence'])
    #yay dataframes, we can select rows and columns by their names
    #we'll store the camera images as our input data
...
def main():
    ...
    #Total_Model = load_model('model-018.h5')
