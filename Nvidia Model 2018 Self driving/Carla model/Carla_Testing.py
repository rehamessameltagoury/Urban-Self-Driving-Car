...
from utils3Cam2018ClassSoft import Append_Sequence
import glob
import os
import sys
import pandas as pd 
import csv
import cv2
sys.path.append("../../../carla/dist/carla-0.9.5-py3.7-win-amd64.egg")
try:
...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None: #x=134.1, y=-201.3, z=5 Location in Town04 ,(x=191.7, y=218.1, z=5) Town02
            transformm = carla.Transform(carla.Location(x=170.2, y=302.6, z=5), carla.Rotation(yaw=180))
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        self._info_text = [
            ...
            'Predicted Speed:   % 20s km/h' % (speed),
            'Predicted Throttle:   % 20s' % (throttle),
            'Predicted steering angle:  % 20s ' % (steering_angle),
...
class CameraManager(object):
    ...
    def _parse_image(weak_self , image):
        ...
        if self.recording:
            ...
            image = utils3Cam2018ClassSoft.preprocess(image)
            ...
            print("Steer out :" + str(Predicted_steering))
            #print("speed" + str(Predicted_speed.shape))
            #print("steer"+ str(Predicted_steering.shape))
            #Predicted_steering     = model2.predict([image ],batch_size = 1)
            #steering_angle = float("{:.3f}".format(Predicted_steering[0][0]))
            steering_angle =float(Predicted_steering[0][0]-1.0)

            temp = Predicted_speed[0][0]
            print("Decelrating = " + str(Predicted_speed[0][0]))
            print("maintain = " + str(Predicted_speed[0][1]))
            print("Accelrating = " + str(Predicted_speed[0][2]))
            for i in range(1,3):
            ...
            throttle = 1.0 - float(Predicted_steering[0][0])**2 - (float(speed)/speed_limit)**2
            ...
            if throttle >= 0.5:
            	...
            	throttle = 0
            print("Predicted steer= "+str(float(steering_angle)))
            print("Predicted speed= "+str(float(speed)))
            print("Predicted throttle= "+str(float(throttle)))
            controller.apply_new(float(steering_angle),float(0.4),float(speed))
            speed_sequence = Append_Sequence(speed_sequence , speed , 10)
