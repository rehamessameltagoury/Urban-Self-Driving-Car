...
np.random.seed(0)
data_df=[]   #Creating an empty list to store dataset inside it then take slices from it 
S=[]
...
def load_data(args):
    ...
    x_Images = data_df[['center', 'left', 'right']].values             #center left right images 
    x_Speed_Sequence  = data_df['Speed_Sequence'].values               #lists of last ten values of speed as a history of speed
    y_steer = data_df['steering'].values	                           #Steering angle labels 
    y_speed = data_df['Speed_Classes'].values						   #Acceleration labels 
 
    #now we can split the data into a training (80%) validation set(20%)
    #thanks scikit learn
