...
def load_data(args):
    ...
    data_df = pd.read_csv(os.path.join(os.getcwd(), args.data_dir, 'comdataandpix.csv'), names=['index','center' ,'left','right', 'steering' ,'Throttle'  ,'brake','Speed', 'Speed_Sequence','Speed_Classes'])
    #using dataframes, we can select rows and columns by their names
    #we'll store the camera images as our input data0                                                                                 

    #x_Images = data_df['center'].values
    index  = data_df['index'].values
    x_Images = data_df[['center', 'left', 'right']].values             #Load images of different cameras 
    x_Speed_Sequence  = data_df['Speed_Sequence'].values               #Load speed sequences as input
    y_steer = data_df['steering'].values                               #Load steering angels as labels 
    y_speed = data_df['Speed_Classes'].values                          #Load acceleration classes as Labels
...
#This function is made to compute different timing of output of the
...
def main():
    ...
    """
    print("This code is to train the model base on a given dataset. ")
    data_bath = input("Please enter path of the dataset.CSV file :")
    Number_of_Epochs = int(input("Please enter the Number of epochs :"))
    samples_per_epoch = int(input("Please enter the samples per epoch :"))
    batch_size = int(input("Please enter f the batch size :"))
    ValidationBatch_Size = int(input("Please enter the Validation batch size  :"))

    parser = argparse.ArgumentParser(description='Behavioral Cloning Training Program')

    parser.add_argument('-d', help='data directory',        dest='data_dir',          type=str,   default=data_bath)
    parser.add_argument('-t', help='test size fraction',    dest='test_size',         type=float, default=0.2)
    parser.add_argument('-k', help='drop out probability',  dest='keep_prob',         type=float, default=0.5)
    parser.add_argument('-n', help='number of epochs',      dest='nb_epoch',          type=int,   default=Number_of_Epochs)
    parser.add_argument('-s', help='samples per epoch',     dest='samples_per_epoch', type=int,   default=samples_per_epoch) #150
    parser.add_argument('-b', help='batch size',            dest='batch_size',        type=int,   default=batch_size) #128
    parser.add_argument('-v', help='batch size',            dest='ValidationBatch_Size',type=int,   default=ValidationBatch_Size) #128
