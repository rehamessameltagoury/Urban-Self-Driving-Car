...
class DualControl(object):
    ...
    def _parse_vehicle_wheel(self ,keys, milliseconds ):
        ...
        if keys[K_LEFT] or keys[K_a]:
            ...
            self._steer_cache = 0.0
        #self._steer_cache = min(0.7, max(-0.7, steering_angle))
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.recording:
            ...
            file = '_out2/%08d' % image.frame_number
            image = Image.open(str(file) + '.png')
            image = np.asarray(image)
            image = utils2016.preprocess(image) 
            image = np.array([image])
            steering_angle = float(model.predict(image, batch_size=1))
            control2.steer = steering_angle
